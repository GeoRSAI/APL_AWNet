...
def conv3x3MW(in_planes: int, out_planes: int, stride: int = 1, groups: int = 1, dilation: int = 1) -> MetaConv2d:
    """3x3 convolution with padding"""
    return MetaConv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=dilation, groups=groups, bias=False, dilation=dilation)

def conv1x1MW(in_planes: int, out_planes: int, stride: int = 1) -> MetaConv2d:
    """1x1 convolution"""
    return MetaConv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False)

class BasicBlockMW(MetaModule):
    ...
    def __init__(
        ...
        super(BasicBlockMW, self).__init__()
        ...
        self.conv1 = conv3x3MW(inplanes, planes, stride)
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3MW(planes, planes)
...
class BottleneckMW(MetaModule):
    ...
    def __init__(
        ...
        super(BottleneckMW, self).__init__()
        if norm_layer is None:
            norm_layer = MetaBatchNorm2d
        width = int(planes * (base_width / 64.)) * groups
        # Both self.conv2 and self.downsample layers downsample the input when stride != 1
        self.conv1 = conv1x1MW(inplanes, width)
        self.bn1 = norm_layer(width)
        self.conv2 = conv3x3MW(width, width, stride, groups, dilation)
        self.bn2 = norm_layer(width)
        self.conv3 = conv1x1MW(width, planes * self.expansion)
...
class ResNetforMW(MetaModule):

    def __init__(
        self,
        block: Type[Union[BasicBlockMW, BottleneckMW]],
        layers: List[int],
        ...
        super(ResNetforMW, self).__init__()
        if norm_layer is None:
            norm_layer = MetaBatchNorm2d
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, BottleneckMW):
                    nn.init.constant_(m.bn3.weight, 0)  # type: ignore[arg-type]
                elif isinstance(m, BasicBlockMW):
                    nn.init.constant_(m.bn2.weight, 0)  # type: ignore[arg-type]

    def _make_layer(self, block: Type[Union[BasicBlockMW, BottleneckMW]], planes: int, blocks: int,
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1MW(self.inplanes, planes * block.expansion, stride),
...
class AFP(nn.Module):
    def __init__(self):
        ...
        self.branch3_1 = nn.Sequential(
            ...
            nn.Conv2d(1024, 1024, 3, padding=1),  # 3_1分支, 先用1×1卷积压缩通道维数, 然后使用两个3×3卷积进行特征提取, 由于通道数为3//2, 此时输出维度设为1
            nn.Conv2d(1024, 1024, 3, padding=1),
        )

        self.branch3_2 = nn.Sequential(
            nn.Conv2d(2048, 1024, 1),  # 3_2分支, 由于1×1卷积压缩通道维数减半, 但是这儿维度为3, 上面用的1, 所以这儿输出维度设为2
            nn.Conv2d(1024, 1024, 3, padding=1)
        )
        # 注意力机制
        self.branch_SE = SEblock(channel=2048)
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        # self.ca2 = eca_layer(1024, 3)
        # self.se2 = SELayer(1024, 16)
        # 网络的第3层加入注意力机制
        # self.ca3 = ChannelAttention(self.inplanes)
        # self.sa3 = SpatialAttention()

        self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2])
        # self.ca3 = eca_layer(2048, 3)
        # self.se3 = SELayer(2048, 16)
        # self.ca4 = ChannelAttention(self.inplanes)
...
def _resnetMW(
    ...
    **kwargs: Any
    ...
    model = ResNetforMW(block, layers, **kwargs)
    if pretrained:
        ...
        update_dict = {k: v for k, v in state_dict.items() if k in fine_tune_net_dict.keys()}

        update_dict.pop('fc.weight')
        update_dict.pop('fc.bias')

        fine_tune_net_dict.update(update_dict)

        model.load_state_dict(fine_tune_net_dict)
...
def resnet50MW(pretrained: bool = True, progress: bool = True, **kwargs: Any) -> ResNetforMW:
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnetMW('resnet50', BottleneckMW, [3, 4, 6, 3], pretrained, progress,
                   **kwargs)
...
def myResNetAvg(model):

    # modelavg = nn.Sequential(*list(model.children())[:-1])
