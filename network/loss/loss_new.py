...
class SCELoss(torch.nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        # Loss
        loss = (torch.tensor(self.alpha).to(device) * ce + torch.tensor(self.beta).to(device) * rce).mean()
...
class ACE(torch.nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        rce = (-1*torch.sum(pred * torch.log(label_one_hot), dim=1))
        # Loss
        loss = (torch.tensor(self.alpha).to(device) * ce).mean() + (torch.tensor(self.beta).to(device) * rce).mean()
        return loss

class ACE_nce(torch.nn.Module):
    def __init__(self, alpha, beta, num_classes=10):
        super(ACE_nce, self).__init__()
        self.device = device
        self.alpha = alpha
        self.beta = beta
        self.num_classes = num_classes
    ...
    def forward(self, pred, labels):
        ...
        # Loss
        loss = (torch.tensor(self.alpha) * nce).mean() + (torch.tensor(self.beta) * rce).mean()
        return loss
...
class ReverseCrossEntropy(torch.nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        return (torch.tensor(self.scale).to(device) * rce).mean()
...
class NormalizedCrossEntropy(torch.nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        return (torch.tensor(self.scale) * nce).mean()
...
class MeanAbsoluteError(torch.nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        return (torch.tensor(self.scale) * mae).mean()
...
class NormalizedFocalLoss(torch.nn.Module):
    ...
    def forward(self, input, target):
        ...
        loss = torch.tensor(self.scale) * loss / normalizor
